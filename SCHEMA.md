...
| `id` | uuid PK | |
| `tenant_id` | uuid FK | |
| `reference` | text | short human code — `K7QMB` — shown in the WhatsApp message. **Unique per tenant:** `unique (tenant_id, reference)`. The seller matches the WhatsApp message against it, so collisions are not cosmetic. Generated by `place_order` as five characters drawn at random from `23456789ABCDEFGHJKMNPQRSTVWXYZ` — chosen for a seller reading it aloud, so no `I` or `L` against `1` and no `O` against `0` — and retried against the unique index on collision. **Never derived from a count.** A sequential scheme needs to read rows the buyer cannot see, which means a `SECURITY DEFINER` counter granted to `authenticated`, which means any storefront visitor can read any tenant's order volume. Earlier references (`A47`, `Q4-B1`) keep their format; uniqueness is per tenant, so the two coexist and nothing renumbers. |
