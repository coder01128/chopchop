-- 0012 — Enforce stock limits at order time for counted tenants
--
-- Previously place_order checked only the `available` toggle, and the comment
-- at the top of 0008 explained why: "Two buyers can order the last box; the
-- seller sorts it out on WhatsApp." That was fine when every shop counted stock
-- loosely. Now that the storefront caps the quantity selector to available
-- stock, the server needs to back-stop stale carts — a buyer whose page loaded
-- when stock was 4 should not place an order for 8 after three were confirmed.
--
-- This is a soft gate, not a hard lock: no SELECT … FOR UPDATE, no decrement.
-- Two concurrent orders can still each claim the last unit. Stock is still
-- decremented only at confirm (0007). What this prevents is the obvious case:
-- a single order whose line quantity exceeds the current count.
...
begin
  ...
  loop
    ...
    -- Counted tenants: refuse when the requested quantity exceeds current stock.
...
comment on function public.place_order(uuid, jsonb, jsonb) is
  'Writes a buyer''s order and its lines in one transaction. Prices and labels are read from variants, never from the payload. On a counted tenant, refuses any line whose quantity exceeds current stock. SECURITY INVOKER: the buyer''s own policies apply.';
